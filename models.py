...
def get_gp_rhizoc():
  ...
  def rhizoc_model(index_points):
    kernel_log_amplitude = ed.Normal(loc=0., scale=2., name='kernel_log_amplitude')

    kernel_log_lengthscale = ed.Normal(
        loc=0., scale=2., name='kernel_log_lengthscale')
    ...
    mean_log_rate = ed.Normal(
        loc=0., scale=3., name='mean_log_rate')
    observation_noise_log_variance = ed.Normal(
        loc=-2., scale=1., name='observation_noise_log_variance')
...
def get_gp_classification(dataset='iris', validate_args=False):
  ...
  def gp_classification_model(index_points):
    ...
    kernel_log_amplitude = ed.Normal(loc=0., scale=3.,
                                     name='kernel_log_amplitude',
                                     validate_args=validate_args)

    kernel_log_lengthscale = ed.Normal(loc=0., scale=3.,
    ...
    observation_noise_log_variance = ed.Normal(
        loc=0., scale=3., name='observation_noise_log_variance',
        validate_args=validate_args)

    mean_log_odds = ed.Normal(
        loc=0., scale=3., name='mean_log_odds',
        validate_args=validate_args)
...
def get_gp_poisson(dataset='rongelap'):
  ...
  def gp_poisson_model(index_points):
    kernel_log_amplitude = ed.Normal(loc=0., scale=2.,
                                     name='kernel_log_amplitude')
    kernel_log_lengthscale = ed.Normal(
        loc=0., scale=3., name='kernel_log_lengthscale')
